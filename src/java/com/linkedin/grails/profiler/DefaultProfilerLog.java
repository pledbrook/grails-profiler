...
public class DefaultProfilerLog implements ProfilerLog, ApplicationContextAware {
    ...
    /**
     * Stores the application context used to load this bean. This
     * should not be called once the bean has been instantiated and
     * initialised.
     */
    ...
    /**
     * Sets which appenders should be used by this profiler log. The
     * given list contains the bean names for the corresponding appenders.
     * This should not be called once the profiler log bean has been
    ...
    public void startProfiling(String label) {
        ...
        // Do we have any appenders on the current thread? If not, get
        // them now.
        if (this.appenders.get() == null) {
            // Create a list for all the appenders that we need.
            List<ProfilerAppender> localAppenders = new ArrayList<ProfilerAppender>(this.appenderNames.size());
     ...
     * @param name The name of the method, action, or whatever that is
     * being "entered".
     */
    public void logEntry(Class clazz, String name) {
        // Only log the even if profiling is on.
        if (isProfiling()) {
            // Save the current time and then log the event to all the
            // configured appenders.
            long entryTime = System.currentTimeMillis();
            for (ProfilerAppender appender : this.appenders.get()) {
                appender.logEntry(this.profilingLabel.get(), clazz, name, entryTime);
            }
        }
    }
    ...
    public void logExit(Class clazz, String name) {
        ...
        if (isProfiling()) {
            ...
            for (ProfilerAppender appender : this.appenders.get()) {
                appender.logExit(this.profilingLabel.get(), clazz, name, exitTime);
            }
        }
    }
}
